...
def path_sum_xviii(root: Optional[TreeNode], target_sum: int) -> List[List[int]]:
    """
    Find all paths that sum to a given value using BFS with path tracking and memoization.
    ...
    while queue:
        ...
        # Process children
        if node.left:
            new_path = current_path + [node.left.val]
            queue.append((node.left, new_path))
        if node.right:
            new_path = current_path + [node.right.val]
...
def test_path_sum_xviii():
    """Test cases for the binary tree path sum XVIII solution."""
