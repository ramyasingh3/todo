...
def path_sum_x(root: Optional[TreeNode], target_sum: int) -> List[List[int]]:
    """
    Find all paths that sum to a given value using BFS.
    ...
    result = []
    queue = deque([(root, 0, [])])  # (node, current_sum, current_path)
    
    while queue:
        node, current_sum, current_path = queue.popleft()
        ...
        # Enqueue children
        if node.left:
            queue.append((node.left, current_sum, current_path))
        if node.right:
            queue.append((node.right, current_sum, current_path))
...
def test_path_sum_x():
    """Test cases for the binary tree path sum X solution."""
