...
def path_sum_xxvi(root: Optional[TreeNode], target_sum: int) -> List[List[int]]:
    """
    Find all paths that sum to a given value using BFS with prefix sums and path tracking.
    ...
    # Queue elements: (node, current_path, prefix_sums)
    queue = deque([(root, [], {0: 1})])
    
    while queue:
        node, current_path, prefix_sums = queue.popleft()
        ...
        if node.left:
            ...
            queue.append((node.left, current_path.copy(), new_prefix_sums))
        
        if node.right:
            new_prefix_sums = prefix_sums.copy()
            new_prefix_sums[current_sum] = new_prefix_sums.get(current_sum, 0) + 1
            queue.append((node.right, current_path.copy(), new_prefix_sums))
...
def test_path_sum_xxvi():
    """Test cases for the binary tree path sum XXVI solution."""
