...
def path_sum_xix(root: Optional[TreeNode], target_sum: int) -> List[List[int]]:
    """
    Find all paths that sum to a given value using DFS with path tracking and memoization.
    ...
    def dfs(node: Optional[TreeNode], current_path: List[int], path_sums: Dict[int, List[int]]) -> None:
        if not node:
            return
        ...
        # Process children
        dfs(node.left, current_path, path_sums)
        dfs(node.right, current_path, path_sums)
        
        # Backtrack
        current_path.pop()
    
    dfs(root, [], defaultdict(list))
    return result
...
def test_path_sum_xix():
    """Test cases for the binary tree path sum XIX solution."""
