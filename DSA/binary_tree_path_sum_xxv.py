...
def path_sum_xxv(root: Optional[TreeNode], target_sum: int) -> List[List[int]]:
    """
    Find all paths that sum to a given value using DFS with memoization and path tracking.
    ...
    def dfs(node: Optional[TreeNode], current_path: List[int], prefix_sums: Dict[int, int]) -> None:
        if not node:
            return
        ...
        if node.left:
            ...
            dfs(node.left, current_path.copy(), new_prefix_sums)
        
        if node.right:
            new_prefix_sums = prefix_sums.copy()
            new_prefix_sums[current_sum] = new_prefix_sums.get(current_sum, 0) + 1
            dfs(node.right, current_path.copy(), new_prefix_sums)
...
def test_path_sum_xxv():
    """Test cases for the binary tree path sum XXV solution."""
